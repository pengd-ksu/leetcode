class Solution:
    def maxPoints(self, points: List[List[int]]) -> int:
        ...
        if len(points) == 1:
            return 1
        tan_dict = defaultdict(set)
        for i in range(len(points) - 1):
            for j in range(i + 1, len(points)):
                tan = (points[i][1] - points[j][1]) / \
                (points[i][0] - points[j][0]) if points[i][0] != points[j][0] \
                else float('inf')
                tan_dict[(tan, tuple(points[i]))].add(tuple(points[j]))
                tan_dict[(tan, tuple(points[i]))].add(tuple(points[i]))
                # including itself, since it's set, won't contain duplicates
        return max([len(v) for v in tan_dict.values()])
