class Solution:
    def uniqueLetterString(self, s: str) -> int:
        ...
        res = 0
        for i, c in enumerate(s): #First loop handles all the c except last
            k, j = index[c]
            res += (i - j) * (j - k)
            index[c] = [j, i]
        for c in index:# Second loop handles the last one to the end
            k, j = index[c]
            res += (len(s) - j) * (j - k)
        return res
...
a substring.
In other words, there are only 6 substring, in which this A contribute 1 point 
as unique string.
...
to index[c].
Count when loop. For example, if "A" appears twice at index 3, 6, 9 seperately, 
we need to count:
